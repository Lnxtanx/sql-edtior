...
export function UsageFooter() {
    ...
    const { usage, refetch } = useUserProfile();
    ...
    return (
        <>
                        ...
                        {!isPro && (
                            <div className="pt-2">
                                <Button
                                    size="sm"
                                    variant="secondary"
                                    className="w-full h-8 text-xs bg-blue-600 text-white hover:bg-blue-700 border-0"
                                    onClick={() => {
                                        setOpen(false);
                                        setShowPricing(true);
                                    }}
                                ...
                                <p className="text-[10px] text-slate-400 text-center mt-2">
                                    Get 8,000 credits/mo + more models
                                </p>
                            </div>
                        )}
                        ...
                        {isPro && (
                            <div className="pt-2">
                                <Button
                                    size="sm"
                                    variant="outline"
                                    className="w-full h-8 text-xs border-border bg-card hover:bg-muted text-foreground"
                                >
                                    Manage Subscription
                                </Button>
                            </div>
            ...
            <PricingModal
                ...
                onSuccess={() => {
                    refetch();
                    toast.success("Thanks for upgrading!");
                }}
            />
        </>
    );
